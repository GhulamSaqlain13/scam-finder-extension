...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  if (message?.type === "FSD_CLEAR" && !sender.tab?.url?.startsWith("https://")) {
    queue = queue.then(async () => {
      await chrome.storage.local.remove(["fsd_history", "fsd_last_result", "fsd_highest_result"]);
      return { ok: true };
    });
    ...
    const results = Array.isArray(message.results) ? message.results.filter(result =>
    ...
    queue = queue.then(async () => {
      const data = await chrome.storage.local.get(["fsd_enabled", "fsd_keep_history", "fsd_history", "fsd_last_result", "fsd_highest_result"]);
      if (!data.fsd_enabled) return { ok: false };
      const update = { fsd_last_result: results[results.length - 1] };
      const candidates = [data.fsd_highest_result, data.fsd_last_result, ...results].filter(Boolean);
      update.fsd_highest_result = candidates.reduce((highest, result) => result.score >= highest.score ? result : highest);
