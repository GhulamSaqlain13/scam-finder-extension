...
let history = [];
threshold.addEventListener("input", () => { document.getElementById("threshold-value").textContent = threshold.value + "%"; });
async function loadHistory() {
  ...
  for (const result of [...history].reverse()) {
    ...
    const score = document.createElement("strong"); score.textContent = result.score + "% risk";
    const time = document.createElement("small"); time.textContent = new Date(result.checkedAt).toLocaleString();
    const signals = document.createElement("p"); signals.textContent = result.signals.join(" · ") || "No rule matched.";
...
chrome.storage.local.get(["fsd_threshold", "fsd_keep_history"]).then(data => {
  threshold.value = data.fsd_threshold ?? 30;
